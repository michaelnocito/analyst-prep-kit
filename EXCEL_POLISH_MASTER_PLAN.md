...
- **Phase G — AI Coach (decision-gated, premium).** Modes 1 & 3 ✅ complete (Excel, v1.83.7). Mode 2 (mock interview) held until after Phase H — designed once for all kits simultaneously. **Effort: Large, multi-cycle.**
- **Phase H — Port to other kits (structure + content review) — runs BEFORE Phase G Mode 2.** Two-part work per kit, done in this order every time:
  - **H2 first — Content review:** For each kit, before touching structure, run a full lesson content audit: (1) **Accuracy** — is every lesson technically correct and up to date? (2) **Relevance** — are the examples real tasks an analyst would do on the job? (3) **Friction** — is any wording confusing, jargon-heavy, or unclear on first read? (4) **Style consistency** — does the kit read in one voice, matching the mentor-tone established in Excel? Fix gaps before porting structure so bad content doesn't get a new coat of paint. 🔒 LOCKED RULE.
  - **H1 second — Structural port:** Apply the proven Excel v2 flow (Orient→Worked Example→Try→Compare→Build→Own→Close) + AI Coach Modes 1 & 3 + all items from the Cross-kit changes log. ⚠️ Per-kit data shapes differ — see CLAUDE.md cheat-sheet. 🔒 LOCKED RULE: content review always precedes structural port.
  - **Kit order:** SQL → Python → Power BI → Tableau → Stats. **Effort: Large (per kit).**

**Revised order (locked June 30, 2026):** A → B → C → D → E → F → G (Modes 1+3) → **H (all kits, content review first)** → G Mode 2 (mock interview, all kits at once) → Data Migration Audit.
