...
- **Phase H — Port to other kits (structure + content review).** Two-part work per kit, done together in one cycle:
  - **H1 — Structural port:** Reuse the proven Excel v2 flow (Orient→Worked Example→Try→Compare→Build→Own→Close) on SQL · Python · Power BI · Tableau · Stats, one kit per cycle. ⚠️ Per-kit data shapes differ — see CLAUDE.md cheat-sheet. Includes the cross-kit jumbled-title sweep and all items from the Cross-kit changes log below.
  - **H2 — Content review:** For each kit, before porting the structure, run a full lesson content audit: (1) **Accuracy** — is every lesson technically correct and up to date? (2) **Relevance** — are the examples real tasks an analyst would do on the job? (3) **Friction** — is any wording confusing, jargon-heavy, or unclear on first read? (4) **Style consistency** — does the kit read in one voice, matching the mentor-tone established in Excel? Fix gaps before porting the structure so bad content doesn't get a new coat of paint. **Effort: Large (per kit).**
