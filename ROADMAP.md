...
>
> **BLOCKER — ✅ FIXED v1.174.0 (2026-07-22):**
> - **[S3] Mobile tap-to-add order is reversed.** ✅ **SHIPPED.** Diagnosis corrected two earlier assumptions: (1) the tap handlers were never wrong — all kits append in tap order, confirmed by driving the LIVE site at 375px before touching code; (2) it is **not** a 6-kit class defect. Real cause, **SQL practice drill only**: after a wrong check `scaffoldParsons()` restacked the columns (answer jumped above the pool) *and* returned the learner's placed lines to the pool to seat the correct first half — so a second tap appeared to leapfrog the first. Restack deleted; column order is now stable for the whole exercise. Also shipped with it: **↑ ↓ nudge controls on placed pieces across all 7 tap-to-order surfaces** (sql · python · powerbi · tableau · stats lesson stages + sql & tableau practice drills), answering Mike's "I'm not sure if we can put them movable"; touch-accurate scaffold copy (was "Drag the rest into order"); Tableau's practice drill stacks to one column under 600px (was two ~140px columns of code at 375px). Excel n/a — no tap-to-order surface. Verified at 375px + 1280px on every changed surface, console clean.
>
> **BUG / verify-live — ✅ BOTH FIXED v1.175.0 (2026-07-22). Neither was a stale deploy; the earlier "source is already correct" reads were looking at the wrong function.**
> - **[T6] Tableau quick-recall shows no answers (vs Excel).** ✅ **SHIPPED.** Not a stale build and not a reveal bug. `_queueRecalls()` stores a **snapshot** of the cue objects (`cues: l.reinforces`) when a lesson is visited, so cues queued before a kit authored answers stay bare strings forever and can never gain one — Tableau's answers landed in v1.164.0 (later than Excel's), so Mike's older Tableau queue was string-only. `_recallsHTML()` now resolves cues from the lesson's CURRENT `reinforces` via the entry's `srcId`, falling back to the snapshot only if that lesson is gone. All 6 lesson kits. Verified by seeding a legacy string-only queue in Tableau + Excel: both now reveal real answers, no "predates stored answers".
> - **[S1] "Reset all progress" doesn't clear the review list.** ✅ **SHIPPED.** The earlier finding checked `resetProgress()` (per-kit) — Mike used **"Reset all kits"** (`handleResetAll`), whose key list was hand-maintained and had drifted: **Tableau and Stats listed NO recall keys**, Excel only its own, Python three kits'. Replaced with a prefix sweep over every kit's key family (`*-recalls`, `*-recall-wins`, `*-last-visit`, `*-streak`). SQL + Power BI have no such button (their `resetProgress()` was already correct) — untouched. **A reset still never clears `apk-pass` (purchase), `apk-coach-key`, or `sim2-apikey`** — guarded and verified.
> - **[T4] Mobile white-screen delay on the right/wrong → congrats/correction transition.** ✅ **FIXED v1.176.0 (2026-07-22).** Not a perf problem and not a 6-kit item. Tableau's `celebrate()` applied the full-screen overlay class to **`<body>`** for 600ms instead of to the `<div id="flash">` the kit already ships. `.flash` is `position:fixed;inset:0`, so the whole page got pinned to the viewport: measured live at 375px, `document.scrollHeight` collapsed **2892→812**, `<body>` went `static→fixed`, and scroll was forced from 400 **to 0 and never restored** — content below the fold blanked and you landed at the top. Now tints the `#flash` div like Excel does; verified body stays static, layout holds, scroll is preserved, tint resolves to `opacity:.08`. Side effect fixed too: the lesson-complete `scrollTo({behavior:'smooth'})` finally has somewhere to animate from. **Tableau-only, verified:** excel+python already tint the `#flash` div · stats scopes it to `body.flash::after` (pseudo-element, no layout impact) · sql+powerbi have no flash. Wrong-answer path was never affected (only the `gold` branch added the class).
>
...
> **→ Batch 5 kit rollout (decided 2026-07-16): port path map + badges + return
...
- **INTERFACE CONSISTENCY: audit all kits against a reference kit, smooth cross-kit friction without losing each kit's identity** _(added July 23, 2026 — 1:02 PM ET)_
  - _What:_ Right now each of the 6 lesson kits (SQL, Excel, Python, Stats, Tableau, Power BI) plus the cert/other pages evolved its own take on the orientation elements every user needs on arrival and throughout a visit — nav layout, home/hub entry point, progress indicator style, header structure, "where am I / what's next" affordances. Mike's read: **SQL is the strongest of the current kits** and should serve as the reference. Audit every kit against it, identify which orientation/navigation elements differ and which of those differences are friction (accidental drift) vs. which are legitimate identity (the kit's own content, subject-appropriate art/flow). Mirror the reference for the friction items; leave the identity items alone.
  - _Why it's here:_ Vision #1 ("see your progress at a glance") and Vision #2 ("always know what to do next") depend on the learner's orientation muscle-memory transferring between kits — a learner who's used SQL and opens Power BI shouldn't have to re-learn where the nav lives or how progress is shown. This is the same "SQL is the reference" principle already in use for the Zinc & Sky color migration (see the Guided Knowledge Path item above); this item extends that same reference-kit approach from color to layout/orientation/navigation.
  - _Scope:_
    1. **Catalog the reference:** document SQL kit's orientation elements precisely — nav/header structure, home entry point, progress indicator (stage dots, streak, readiness score), lesson→practice transition pattern, "what's next" CTA placement, mobile nav behavior.
    2. **Audit the other 5 kits + cert/other pages** against that catalog; note every divergence.
    3. **Classify each divergence:** friction (unintentional inconsistency that costs the learner orientation time) vs. identity (deliberate, subject-appropriate difference — e.g. Tableau's Viz-Builder surface, SQL Quest's battlefield chrome, Excel's Pivot Lab — these stay put per the "games/domain surfaces keep their own art" precedent already set in the Zinc & Sky migration).
    4. **Mirror the friction items** onto every kit; leave identity items untouched.
  - _Definition of Done:_ A learner moving between any two lesson kits recognizes the same nav pattern, home entry point, and progress/orientation affordances without re-learning them, while each kit's subject-specific identity and content flow is preserved. Verified live across all 6 kits, dark + light, desktop + mobile.
...
- **GR-G (EPIC): "Guided Path" — merge Learn + Practice into one linear flow, simplify home, tone down Bare Basics, Excel makeover** _(added June 1, 2026 — 12:40 AM ET — TOP PRIORITY per Mike; prototype on Excel first)_
  ...
  - **➕ July 23, 2026 addendum (1:20 PM ET) — Thread 1 has a residual seam; research-backed redesign needed, NOT YET BUILT.** Mike's fresh playtest note: moving from a lesson into its practice cards, and from practice back into the next lesson, still *feels* like two separate completed things with a mental pause in between — even though Thread 1 already collapsed the Lessons/Practice/Cards tabs into one guided route. **Root cause, confirmed in SQL kit source (the reference kit):** `finishLesson()` (sql/index.html) appends a manual **"Continue path →"** button into the Quick Check result before `startGuided()` fires, and `startGuided()`/`renderGuidedStep()` then swaps `#main` into a **differently-framed page** (its own header/back-button/dot style, distinct from the lesson's own v2 stage-bar frame) to walk through 3 recall cards + 3 drills, ending in **"Finish — next lesson →."** So the seam isn't a literal "You're done!" screen, but a required manual click plus a visual modality-switch between two different frames — which reads as the same kind of break.
  - **Research pass done (deep-research harness, 102 agents, 20 sources fetched, 25 claims adversarially verified 16 confirmed/9 refuted, July 23 2026):**
    - Fading a worked example directly into problems (tight coupling) beats the traditional blocked worked-example-then-separate-problem structure on near-transfer outcomes, replicated in real classrooms and lab studies (Renkl, Atkinson, Maier & Staley 2002, *Instructional Science*, DOI 10.1023/B:TRUC.0000021815.74806.f6; Paas, Renkl & Sweller 2003, *Educational Psychologist*).
    - For learners with even moderate prior knowledge (our adult analyst-skills audience), a **fast/immediate** transition into independent practice beats a slow, heavily-scaffolded fade — only true beginners benefit from more gating (Reisslein 2005/2007, *Journal of Engineering Education*). This is an expertise-reversal boundary condition, not a blanket "always instant."
    - The benefit of tight coupling comes from **reducing unproductive behavior at the transition seam** — not from more self-explanation, which was tested directly and found to have no effect (Renkl, Atkinson & Grosse 2004). This is the closest primary evidence that friction *at the seam itself* (a required click, a reframed screen) is the thing costing us, independent of content.
    - CLT: forcing a learner to search across disjointed, non-integrated material imposes extraneous cognitive load with no learning benefit (Paas, Renkl & Sweller 2003) — the best available analogue to a hard screen/frame break, though **no study directly tested a UI completion-screen or click-to-continue pattern**; this is an inference from adjacent findings, not a direct hit.
    - Motivational risk of a *premature* completion signal: the endowed-progress-effect literature shows perceived completion (not actual remaining work) drives follow-through effort (Nunes & Dreze 2006, *Journal of Consumer Research* 32(4):504). Applied here (inference, not a direct instructional-UI study): showing anything that reads as "done" before the practice/retrieval step happens risks signaling false closure and undercutting effort on the practice that follows.
    - Merrill's First Principles of Instruction formally place an Application phase right after Demonstration (Merrill 2002, *Educational Technology Research and Development* 50(3):43-59, DOI 10.1007/BF02505024) — supports practice belonging immediately after the example, but the source itself is silent on screen/frame architecture.
    - **Caveat (from the research, stated plainly):** no peer-reviewed study directly tested a literal "lesson complete" click-through screen — the case rests on adjacent CLT/worked-example/motivation literature, not a direct instructional-software study. Treat the recommendation below as strongly-supported inference, not a proven one-to-one finding.
  - _Recommendation (design target, not yet built):_ (1) Remove the manual "Continue path →" click — the moment the Quick Check resolves, auto-continue straight into the first practice card/drill. (2) Render practice cards/drills **inside the same stable stage frame** the lesson already uses (stage bar + uniform bottom bar), not a separate `#main` page-swap with its own header/dot style — the frame-switch is the modality break, not just the click. (3) Move any celebratory/"lesson complete" visual moment to occur **after** the practice sequence finishes, not at the Quick Check — a checkmark shown before retrieval practice risks the false-closure effect above. (4) Given the expertise-reversal finding, kits/lessons aimed at true beginners can justify a touch more scaffolding at the seam than kits aimed at learners with some prior exposure — worth a design call per kit, not a blanket rule.
  - _Scope:_ Touches the guided-path engine (`startGuided`/`renderGuidedStep`/`finishLesson`) shared across all 6 lesson kits — same "SQL is the reference, Excel pilots first" convention as the rest of this epic.
  - **✅ Excel pilot SHIPPED (July 23, 2026 — 1:44 PM ET, commit 5aca225, live-verified):** `v2CheckOwn()`'s correct-answer branch now skips straight into `startGuided(lid)` when the lesson has drills, instead of revealing the Close stage's "Lesson complete" checkmark card first — one "Continue" click goes straight from the Quick Check into practice. The lesson-done celebration toast moved into `guidedNext()`'s end-of-sequence branch, firing non-blocking (existing `celebrate()` pattern — toast + chime, auto-hides, never gates navigation) right as the learner lands on the next lesson, AFTER the practice round instead of before it. **Concept-only lessons with no drills are unchanged** — they still reveal the Close card normally, since there's no practice to defer past. Verified live in-browser end to end (walked a full lesson: Try → Build → Own → straight into Practice 1/11 with no gate, then all 11 steps → "Lesson complete — nice work! 🎉" toast fires as it lands on the next lesson). **NOT done in this pass:** the deeper "same visual frame" merge (practice still renders in a separate `#view-drill` top-level view, not inside the lesson's own continuous stage flow) — that was the MEDIUM-confidence, inference-only part of the research, left for a later pass if the click+premature-checkmark fix isn't enough on its own.
  - _Next:_ Mike playtests the Excel pilot; if it feels right, port the same `v2CheckOwn`/`guidedNext` pattern to SQL/Python/Power BI/Tableau/Stats (one kit per chat, per convention).
  - _Est. effort:_ Large (one kit per chat, Excel pilot first per convention, then port).
