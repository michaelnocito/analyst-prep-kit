...
**Current version:** `v1.71.0` (shipped June 24, 2026 — 🎨 Grain redesign Phase 2i: **Simulator** (Claude-API) kit restyled to Grain — full var set→tokens, Space Grotesk headings, Lucide `briefcase` brand mark + chrome/intro/stat icons via the robust loader. **Style only — the live Claude API fetch/model/key logic was NOT touched** (verified intact). Two baked tints → `color-mix`; the cool skill-category palette (`TYPE_COLOR`) remapped to Grain hues. No Chart.js. Verified headless, 0 syntax errors, 0 residue, API block confirmed untouched. Awaiting Mike's live playtest.)
