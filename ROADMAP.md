...
**Current version:** `v1.33.0` (shipped June 3, 2026 — awaiting test)
**Recent:** v1.33.0 **visual-parity sweep — Tableau kit DONE** (L5/L6/L8/L12 now show every concept the text names). v1.32.0 L15/L20 parity fixes. v1.31.0 Final Exam study-guide visuals.
**In flight:** visual-parity sweep across all kits (~26 gaps, tracked in `VISUAL_PARITY_SWEEP.md`). Tableau ✅ done. Remaining: **Excel (5) → SQL (4) → Python (7) → Power BI (4) → Stats (2)** — fixing kit-by-kit, one ship per kit. v1.27.0 "Know Your Workspace" UI module. v1.24.0 Cycle 1/7 L14 ✅ PASS.
