...
- **GR-G (EPIC): "Guided Path" — merge Learn + Practice into one linear flow, simplify home, tone down Bare Basics, Excel makeover** _(added June 1, 2026 — 12:40 AM ET — TOP PRIORITY per Mike; prototype on Excel first)_
  ...
  - _Scope:_ Touches the guided-path engine (`startGuided`/`renderGuidedStep`/`finishLesson`) shared across all 6 lesson kits — same "SQL is the reference, Excel pilots first" convention as the rest of this epic. **Needs Mike's go-ahead before build** given the size (multi-kit, touches the core guided-path machinery) — this entry documents the research + recommendation only.
