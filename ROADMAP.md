...
**Current version:** `v1.32.0` (shipped June 3, 2026 — awaiting test)
**Recent:** v1.32.0 **visual-parity fix** — L15 now shows JOIN + UNION + RELATIONSHIP (was join-only); L20 adds a combined-axis visual next to dual-axis. Principle: if the text names co-equal concepts, the visual shows all of them. v1.31.0 Final Exam study-guide visuals. v1.30.0 Tableau cert-critical subset COMPLETE (20 lessons).
**Open (Mike's call):** "menu-style" visual sweep — L5/L6/L8/L12 list options in text but show one example (weaker case; may be fine). Also: extend the visual-parity sweep to the other kits' "See it on screen" visuals? v1.27.0 "Know Your Workspace" UI module. v1.24.0 Cycle 1/7 L14 ✅ PASS.
