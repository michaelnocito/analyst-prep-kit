...
**Current version:** `v1.23.2` (shipped June 3, 2026 — awaiting test)
**Recent:** v1.23.2 Grouping lesson — deeper basics (3 ways to group + same/different, the Data-pane "where did my group go?" gotcha, Group/Set/Hierarchy decision guide), verified vs Tableau docs. v1.23.1 Grouping lesson — added methods + Group-vs-Set-vs-Hierarchy note. v1.23.0 Tableau new **Grouping** lesson (Lesson 9, Unit 2) — combine members into a group, "Group Other," group-vs-everything-else visual + 3 guided tap-the-choice drills (Mike's class topic). v1.21.0 Tableau drills → tap-the-choice; v1.22.0 rolled tap-the-choice to all kits' Bug Hunt + Describe drills (no free-text answer boxes remain anywhere).
