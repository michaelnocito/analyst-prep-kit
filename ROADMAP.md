...
**Current version:** `v1.24.0` (shipped June 3, 2026 — awaiting test)
**Recent:** v1.24.0 Tableau cert beef-up **Cycle 1/7** — new L14 "Live vs. Extract Connections" (Unit 4, exam Domain 1). Plan in `TABLEAU_CERT_PLAN.md`; next up L15 Joins/Unions/Relationships. v1.23.2 Grouping lesson — deeper basics + Data-pane gotcha, verified vs Tableau docs. v1.23.1 Grouping lesson — added methods + Group-vs-Set-vs-Hierarchy note. v1.23.0 Tableau new **Grouping** lesson (Lesson 9, Unit 2) — combine members into a group, "Group Other," group-vs-everything-else visual + 3 guided tap-the-choice drills (Mike's class topic). v1.21.0 Tableau drills → tap-the-choice; v1.22.0 rolled tap-the-choice to all kits' Bug Hunt + Describe drills (no free-text answer boxes remain anywhere).
