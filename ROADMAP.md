...
**Current version:** `v1.74.0` (shipped June 25, 2026 — 📚 **Phase 3 begins:** SQL kit gains Unit 4 "Data Migration" interview track — 8 new lessons (ids 501–508), purely additive, translated from Grain's `lessons-sql.js` into the SQL kit's shape; SQL now 28 lessons. Approach: add tracks only, one kit per cycle. ⚠️ **Pending decision:** SQL Units 5–7 + the other tool kits' tracks use chart viz (bar/line/waterfall/boxplot/combo/scatter) the kits can't draw — need a viz-handling call (render-as-table vs add inline-SVG charts vs omit). Hub "12 lessons" labels are pre-existing stale.)
