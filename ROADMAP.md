...
- **Guided knowledge path (Duolingo-style) — thread the orphaned features into the lesson flow** _(added July 16, 2026 — 5:17 PM ET; from SQL-kit playtest inbox note, July 16)_
  ...
  - _Rollout status:_ ✅ **Excel shipped v1.142.0 (July 16, 2026)** — pattern to copy: `LESSON_CARDS` map + `guidedSeqFor()` + `startUnitReview()` + "Continue path →" CTAs + `S.unitReviewsDone`, see excel/index.html around `LESSON_DRILLS`. Remaining, one kit per chat: **SQL next**, then Python, Power BI, Tableau, Stats (Tableau/Stats use a sections-based lesson render — the guided plumbing exists in tableau but check stats), then evaluate Chart-Literacy/Forecasting (may lack drill arrays entirely).
