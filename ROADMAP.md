...
**Current version:** `v1.106.0` (shipped July 6, 2026 — 🧠 **Power BI kit Phase D+E parity**: spaced-recall engine + 39 cue sets + bridge/honesty/wins/greeting; resume-card position fix; reset clears recall keys. 69 headless checks green across SQL+Python+PBI. Test batch 29 in `apk_CLAUDE.md`. **Remaining: Tableau → Stats.**)
...
- **LEARNING SCIENCE: Restructure Excel kit for spaced retrieval + retrieval practice + hint escalation** _(added June 28, 2026 — 3:45 PM ET)_ — _Phase 1 (High), Excel pilot before rollout_
  - _STATUS (July 6, 2026):_ ✅ **Excel COMPLETE** (master-plan Phases A–F, v1.78.0–v1.83.0) + Phase H ported the v2 structure to all 5 other kits. ✅ **SQL now at FULL D+E parity too (v1.104.0)** — spaced-recall engine + 46 cue sets + bridge + honesty note + wins counter + greeting. **Remaining: roll the same D+E layer to Python → Power BI → Tableau → Stats** (recipe + port checklist in `apk_CLAUDE.md`; ~one kit per cycle, cue authoring is the bulk of each). Testing backlogged: batch 27.
