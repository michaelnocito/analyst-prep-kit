...
- **Guided knowledge path (Duolingo-style) — thread the orphaned features into the lesson flow** _(added July 16, 2026 — 5:17 PM ET; from SQL-kit playtest inbox note, July 16)_
  ...
  - _Rollout status:_ ✅ **Excel shipped v1.142.0 (July 16, 2026)** — pattern to copy: `LESSON_CARDS` map + `guidedSeqFor()` + `startUnitReview()` + "Continue path →" CTAs + `S.unitReviewsDone`, see excel/index.html around `LESSON_DRILLS`. ✅ **SQL shipped v1.143.0 (July 16, 2026)** — same pattern plus lab hand-off steps (`LESSON_LABS`: lesson 4 → JOIN Lab, lesson 5 → Aggregation Lab); all 38 cards mapped once across `LESSON_CARDS`. Remaining, one kit per chat: **Python next**, then Power BI, Tableau, Stats (Tableau/Stats use a sections-based lesson render — the guided plumbing exists in tableau but check stats), then evaluate Chart-Literacy/Forecasting (may lack drill arrays entirely).
