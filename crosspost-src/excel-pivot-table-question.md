...
The example running through this article is the real one from [the build behind this series](https://michaelnocito.github.io/analyst-prep-kit/guides/excel-pivot-table-question/../excel-dashboard-build-order/): 82,956 Steam games, labelled into four groups in [article 1](https://michaelnocito.github.io/analyst-prep-kit/guides/excel-pivot-table-question/../excel-label-rows-before-charting/), and counted by a pivot in step 3 of the build.
...
**The Rows field must be a label.** A column with a small number of repeating values: segment, genre, region, month. Grouping by a column where every value is different, like a game's name, produces 82,956 groups of one, which answers nothing. This is why [labelling came four articles before pivoting](https://michaelnocito.github.io/analyst-prep-kit/guides/excel-pivot-table-question/../excel-label-rows-before-charting/). The label column is what makes the question askable.
...
The question from the build: how many games are in each segment?
  ...
  1. **Click inside the Games table, then Insert > PivotTable.** Because the data is [a named Table](https://michaelnocito.github.io/analyst-prep-kit/guides/excel-pivot-table-question/../excel-name-your-data/), the range is right by construction, and new rows will be included on refresh.
...
Say out loud what the Grand Total row is doing for you. It is [article 2's addition check](https://michaelnocito.github.io/analyst-prep-kit/guides/excel-pivot-table-question/../excel-check-your-work/), delivered without being asked: the groups sum to the row count of the file, so no game was dropped and none was counted twice.
...
**Filters** restricts which rows are allowed to answer. "Only games released after 2015." It is the one zone that removes rows, so everything [article 1 said about filters](https://michaelnocito.github.io/analyst-prep-kit/guides/excel-pivot-table-question/../excel-label-rows-before-charting/) applies: fine for looking, dangerous for comparisons you are about to present.
...
This is also the boundary between a report and an analysis, which [has its own guide](https://michaelnocito.github.io/analyst-prep-kit/guides/excel-pivot-table-question/../report-vs-analysis/): a report states numbers, an analysis answers a question someone will act on. A pivot built from a sentence is already halfway to the second.
...
A pivot is a computed result, and computed results get checked, per [article 2](https://michaelnocito.github.io/analyst-prep-kit/guides/excel-pivot-table-question/../excel-check-your-work/). The build's checks, both cheap:
...
*The full version of this guide lives on my site: [A Pivot Table Is a Question, Not a Report](https://michaelnocito.github.io/analyst-prep-kit/guides/excel-pivot-table-question/).*
