...
**The move:** timebox and switch strategy. The cause was mechanical. Both join keys were wrapped in functions, which disabled any index and forced ~427 million row-by-row comparisons (the full story is in the [large-datasets guide](https://michaelnocito.github.io/analyst-prep-kit/guides/technical-tenacity/../handle-large-datasets/)). The first fix attempt (a fancier index) didn't take either. The second strategy — precompute the cleaned keys into real columns, index those, join plainly — worked in seconds. Two strategy changes, zero heroic waiting.
...
| Log the war story                | Every solved wall goes in the project's data-quality or troubleshooting notes — the [working record](https://michaelnocito.github.io/analyst-prep-kit/guides/technical-tenacity/../documenting-data-limitations/) — so it's a lesson once and never a mystery twice. |
