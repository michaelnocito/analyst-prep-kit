...
**Last session ended:** July 5, 2026 — ✅ **Cleared High-bucket items #1–#4 (v1.98.0 → v1.100.0).** #1 mobile action buttons, #2 mobile text-overflow, #3 nav-overlapping-title (all mobile CSS, all 6 kits, pending Mike's phone check) + **#4 free-text drills → tap-the-choice** (SQL/Python/PBI/Stats, hand-authored distractors, grading-trust; Excel was already done). All parse clean. Working the ROADMAP High bucket one item at a time (Mike's cadence: I build+verify+ship one, hand off, he tests via the spreadsheet). **Key finding:** all drills are now tap-based (chips/MC) — NO free-text inputs — so the "keyboard covers the submit button" framing is moot; the fix = make the primary action a full-width, 48px-min, thumb-height CTA on mobile (≤600px), placed at the bottom of its button row. CSS-only, desktop untouched. **Per-kit gotchas that mattered:** Excel reveals all stages (`.v2-continue`, `.v2-drill-actions`, `.v2-close-actions`); the other 5 render ONE `.v2-stage-card` at a time. SQL/Power BI use `.row` button wrappers (already wrap); Python/Tableau/Stats use inline `<div style="display:flex">` with no wrap → forced via `.v2-stage-card div[style*="display:flex"]{flex-wrap:wrap}`. **Stats' primary is plain `.btn` not `.btn-primary`** (targeted `.btn:not(.btn-outline):not(.btn-sm)`) and had no ≤600px block (added one). All parse clean.
...
**➡️ NEXT = ROADMAP High #5: skill-readiness score ignores the new lessons** — now UNBLOCKED (the #6 decision it depended on is made). **✅ #6 DECISION (Mike, July 5): a lesson = complete when it reaches the v2 "Done"/Close stage** (already requires Try → Build → Quick Check); standalone practice drills stay OPTIONAL and do NOT gate completion. So #5 readiness = **(# lessons that reached Done) / (LIVE lesson count)** per kit.
  **#5 scoping started (July 5):** the kits differ — **Excel** has an explicit "readiness" meter (`_visSkills`, `readinessPct`/`readinessList`, computed ~line 2057) and already got a live-lesson fix in v1.78.0; **SQL has no `readiness` at all** (grep found nothing), so its home shows progress a different way. So #5 is NOT one uniform change — investigate each kit's home progress/readiness indicator, confirm the denominator uses the live lesson count (not a frozen 12/16) and the numerator counts lessons that reached Done. Excel is the reference for the "derive from live lessons" approach. Check: does reaching the v2 Done stage already set the kit's `doneLessons`/`S.done['lesson'+id]`? (SQL/Stats set `S.done['lesson'+id]` at the Done stage — verify each kit marks completion there, since that's now the definition.)
  THEN remaining Medium, THEN Low, THEN Phase G Mode 2. Full bucket list in `ROADMAP.md`.
