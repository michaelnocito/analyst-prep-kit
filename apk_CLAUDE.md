...
**➡️ NEXT = ROADMAP High #4: convert fragile fill-in-the-blank to multiple-choice (audit all kits).** Grading-trust bug — inventory every remaining free-text/type-the-answer input across all kits (guided steps / exam / any Parsons gaps) and convert the ungradable ones to tap-the-choice (keep the "correct = index 0" pattern). NOTE much was already done (v1.21–22 removed free-text from Bug Hunt + Describe; v2 drills are all tap-based) — this is a sweep for stragglers. Then the rest of High (stale readiness score; Check-button completion model = a **decision** item, ask Mike before building), THEN Medium, THEN Low, THEN Phase G Mode 2. Full bucket list in `ROADMAP.md`. **Minor mobile leftovers if Mike flags them:** standalone practice-drill Check buttons (plain mid-card `.btn`) + drag pieces (`.pars-piece`) weren't touched by the #1/#2 passes.
