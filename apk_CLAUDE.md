...
**➡️ NEXT = ROADMAP High #5: skill-readiness score ignores the new lessons.** The readiness meter/denominator doesn't count the lessons added by the 4 interview tracks (SQL 46 / Excel 51 / Python 42 / Power BI 39). Find where readiness is computed per kit (completion ratio over lesson count) and make it derive from the LIVE lesson set so new lessons are included and completing one moves the score. ⚠️ **Coordinate with #6, the next item — it's a DECISION, ask Mike before building:** the "Check button lets you skip a lesson without completing it" completion-model question (does a lesson mark complete on "Check", or only when its drills are done?). #5 and #6 share the same completion data, so get Mike's call on #6's model before finalizing #5's math. THEN Medium, THEN Low, THEN Phase G Mode 2. Full bucket list in `ROADMAP.md`.
