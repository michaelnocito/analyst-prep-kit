...
**➡️ NEXT: the "quick-fix" High items (#1–#6) are all cleared.** What remains in the High bucket is all BIG, decision/research-gated efforts — not quick wins:
  - **AI Coach** (premium; decision-gated — needs Mike's go + 3 decisions on funding/scope/model; research already done, see the ROADMAP entry + `SUPABASE_INTEGRATION.md`).
  - **Learning-Science restructure** (Excel pilot; big multi-system refactor; spec = `EXCEL_POLISH_MASTER_PLAN.md`).
  - **GR-G epic** remaining threads (simplified home + Excel layout makeover — needs Mike's design direction).
  So unless Mike wants to start one of those, the natural next work is the **Medium bucket** (June-29 batch): real chart/pivot visuals in chart exercises · "on the job" blurbs only on track lessons · hub "analyst games" section · mark premium features unlocked post-purchase · attempt-vs-correct free comparison · remove "no login needed" marketing copy · per-kit SEO meta. THEN Low, THEN Phase G Mode 2. Full bucket list in `ROADMAP.md` — **ask Mike which thread to take** (a big High effort vs working the Medium bucket).
