...
**Last session ended:** July 5, 2026 — 📱 **High-bucket items #1 (mobile submit/action button), #2 (mobile text-overflow sweep) AND #3 (nav overlapping the title) — ✅ ALL THREE fixed across the kits (v1.98.0 → v1.99.2). All mobile-UX, CSS-only, pending Mike's phone check.** Working the ROADMAP High bucket one item at a time (Mike's cadence: I build+verify+ship one, hand off, he tests via the spreadsheet). **Key finding:** all drills are now tap-based (chips/MC) — NO free-text inputs — so the "keyboard covers the submit button" framing is moot; the fix = make the primary action a full-width, 48px-min, thumb-height CTA on mobile (≤600px), placed at the bottom of its button row. CSS-only, desktop untouched. **Per-kit gotchas that mattered:** Excel reveals all stages (`.v2-continue`, `.v2-drill-actions`, `.v2-close-actions`); the other 5 render ONE `.v2-stage-card` at a time. SQL/Power BI use `.row` button wrappers (already wrap); Python/Tableau/Stats use inline `<div style="display:flex">` with no wrap → forced via `.v2-stage-card div[style*="display:flex"]{flex-wrap:wrap}`. **Stats' primary is plain `.btn` not `.btn-primary`** (targeted `.btn:not(.btn-outline):not(.btn-sm)`) and had no ≤600px block (added one). All parse clean.
...
**➡️ NEXT = ROADMAP High #4: convert fragile free-text drills → tap-the-choice (grading-trust).** ⚠️ NOT already done. **Precise inventory (audited July 5) — the free-text DRILL inputs that survive in the standalone practice/guided path:**
  - **SQL** `renderFill` → `.fill-input` `#fill-in` + `checkFill()` (~line 2189).
  - **Python** `renderFill` → `.drill-input` `#fillInput` + `checkFill` (~1985) AND `renderEsql` → `#esqlInput` "Write the code…" (~2092). TWO.
  - **Power BI** `renderFill` → `.fill-input` `#fill-in` + `checkFill()` (~2227).
  - **Stats** `renderFill` → `#fillAns` + `checkFill()` (~1817). (Stats `renderEsql` is ALREADY tap — `.bug-choice`/`pickEsql`.)
  - **Excel** = the MODEL, already converted (v1.78.0, "last free-text drill gone, 24 verified distractors"): its FILL/ESQL use `.bug-choice` tap-the-choice, `data-correct`, correct=`choices[0]` then `_shuf`.
  - **Tableau** = nothing to do (its only `<input>`s are the Viz-Builder calc-field editor + searches, legit free-text).
  - **NOT drills anywhere (leave alone):** glossary search, promo/unlock code, coach API-key fields, sliders.
  **The lift = CONTENT, not plumbing:** each FILL/ESQL item in those 4 kits' DATA arrays needs 3–4 authored, verified distractor choices (a right-but-unmatched typed answer marking wrong is the exact bug). Follow Excel's `renderEsql`/`.bug-choice` pattern (tap → `data-correct` → `pickX` → celebrate/markDone; keep `correct=choices[0]` then `_shuf`). Treat like Excel's dedicated pass — author carefully, don't rush distractors. Consider reusing the kit's v2 Build-stage MC choices where the same concept already has them. **Suggest confirming approach with Mike first** (distractor count; reuse-vs-author).
  Then remaining High: **#5 stale readiness score** (denominator ignores the new track lessons — SQL 46/Excel 51/Py 42/PBI 39; make it derive from the live lesson set) and **#6 Check-button completion model = a DECISION item — ask Mike before building** (does a lesson mark complete on "Check", or only when drills done? feeds #5 + Vision #2). THEN Medium, THEN Low, THEN Phase G Mode 2. Full bucket list in `ROADMAP.md`.
  **Minor mobile leftovers if Mike flags them:** standalone practice-drill Check buttons (plain mid-card `.btn`) + drag pieces (`.pars-piece`) weren't touched by the #1/#2 passes.
