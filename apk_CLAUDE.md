...
**Last session ended:** August 21, 2026 — 🎯 **v3.20.0: plan-mode focus (Mike: "we are guiding them then letting them loose with all these options").** In `?from=plan` sittings the wander surfaces hide: drill loses track switcher/break timer/Back/start-over/crumb; kit loses #nav + all "← Lessons" buttons (injected CSS in the plan IIFE). Guides deliberately keep full chrome (Mike allows guides + ads as the only side doors). All hides gated on the query string — native visitors unchanged. ⚠️ Real Test run 011 is SUSPENDED mid-frame on an instrument failure (Browser pane hidden/not compositing + Chrome extension injection timeouts) — persona Tomas + budgets locked, storage-clear-first pending; resume the agent once Mike shows the pane / restarts Chrome. Run 011 should also now expect the v3.20.0 focus mode on drill/kit hops. Prior:

**Prior:** August 21, 2026 — 🩹 **v3.19.0: Real Test run 010 (the plan walk), ALL 12 findings fixed, Mike: "do it in your order, fix all of it".** Headliners: `assets/plan-return.js` (guide/interview/projects pill = the stop AND the box-check, since static pages have no completion event; injected into 12 plan-linked guides + interview + projects — ⚠️ any NEW guide the plan links needs the script tag + ?from=plan on the link); drill `?fresh=1` (clears saved position pre-loader; plan passes it on the four "first…" steps); drill i0 two-tick lock + synchronous boundary check on Next (rt010c/d race); verdict/Next scrollIntoView (rt010e); scroll-to-plan on build + resume (rt010f/l); vocab unified to "drills" (rt010g); overlay dialog a11y both tools (rt010h); per-step aria-labels (rt010i); kit nav wrap→nowrap+overflow-x (rt010j, the v1.99.2 pattern struck again at 788px); NEXT tag lights the first unchecked step (rt010k). Canon §5h gained run 010's cross-check. Tracker: run wrote findings to NEW project row "analyst-prep-kit" (kept deliberately, walk spans kit+drill+guides); fix outcomes written back this session. Prior:

**Prior:** August 21, 2026 — ⏱️ **v3.18.0: sittings timed + raced (Mike: "how fast did they do it, let them race themselves, accuracy, save it all").** ⚠️ Drill bible bans timers → clock is INVISIBLE until the stop screen (post-hoc reward, Deci-safe). Kit: t0 at boot + missesFor(tid) baseline → overlay stat line ("Done in 12 minutes, nothing missed. A clean run.") + `&s=&miss=` on the return URL. Drill: t0 → "3 rungs in 8 minutes" + `&s=&n=`; final-rung pill computes at tap (onclick preventDefault → statHref). Plan: `S.stats[combo][step]={s,at,miss?,n?}` + `S.best[combo:step]` — **both survive Start-over ON PURPOSE** (that IS the race-yourself mechanic; reset copy says "your times stay"). Green act-chip on timed steps ("yours: 12m · clean · best 9m"), pace line in progressline, PB toast, plan_autocheck carries s/miss/rungs/pb → GA4 = population timing data. Real-test agent run on the walk still in flight (path-fluidity §5h added to canon this session); findings go to playtest tracker. Prior:

**Prior:** August 21, 2026 — 🔁 **v3.17.0: the plan loop closes (Mike's 2nd flow finding: "they need to click, do the thing, click complete, back to the checklist, repeat").** Full loop: plan step → tool → tool raises a HARD STOP at exactly the step's scope → one tap home → plan auto-checks that box (`?ckd=<stepId>`) → next step lit. Kit side: plan-mode wraps global `markDone` (fires on replays too — the CALL happens even when markDone no-ops on a dup); stop overlay + green pill at completion. Drill side: `?rungs=` sitting scope (int = relative to resume point, `half` = absolute first half, default `end` = green-pill arm on final rung — final-rung run completion is NOT detectable without hooking `run()`, so `end` mode is one manual tap, by design). Plan side: `&ret=` out / `?ckd=` in (validated `d\d+s\d+` + exists in current combo), badge/comeback parity with manual checks, toast + scroll-to-next, "Day N · x of y" counters, loop described in planhead. ⚠️ SKILLS/cert page unaffected. ⚠️ rungs values live in the plan's `ladder()` calls: '3' ×2 (brandnew d1/d2), 'half' ×3 (brandnew d4/d6, rusty d5), rest default. NEXT: real-test agent gets a path-fluidity/cohesion judging ability, then a live playtest run of this exact loop. Prior:

**Prior:** August 21, 2026 — 🔀 **v3.16.0: plan sittings separated from kit progress (Mike playtest finding).** His report: a plan step linking `sql/#lesson-100` resumed his old kit progress "starting me at the end", and learners who enter the kit get lost and never return for step 2. Fix = kit's own patterns, NOT lesson rebuilds (path/ header records why duplication is wrong). New `#lesson-<id>-plan` deep-link mode in `sql/index.html` (~line 6166 block): clears `state.lessonStage[id]` + parsons attempt so the lesson opens at stage 1, **but never touches `doneLessons`** — the pre-existing `-restart` mode DOES wipe completion (PrepLoop semantics, left untouched). Kit's `?from=path` pill IIFE extended to `?from=plan` (→ `../plan/`, "← Back to your plan"); same opt-in pill added to `drill/index.html` (end of script). Plan page emits `../sql/?from=plan#lesson-<id>-plan` and `../drill/?ladder=<slug>&amp;from=plan`; **drill links deliberately keep resume** (day 2 "next three rungs" depends on it); planhead states the behavior. ⚠️ If more plan targets are added later (interview/, viz/), give them the same `from=plan` pill. Prior:

**Prior:** August 21, 2026 — 📜 **v3.15.0: `plan/certificate/` — the plan-completion certificate.** Mike's go on the flagged v3.14.0 follow-up. **The URL is the certificate** (`?level=&amp;role=&amp;days=&amp;name=&amp;d=`): no accounts, renders client-side, honest wording ("Completion is self-directed and self-reported", verify line links the builder). SKILLS map lists what each plan actually contained, cumulative 3→5→7 — ⚠️ **kept manually in step with the day content in plan/index.html; change one, change the other.** Name typed on-page → `localStorage['apk-plan-name']` → written into the link live (replaceState). Actions all user-initiated: copy link · print/save-PDF (`@media print` strips to the card) · **LinkedIn add-to-profile prefilled with certUrl → every saved cert is a standing backlink** (freeCodeCamp mechanic; cert-as-goal per Goli et al. 2022). Empty/invalid params → explainer + route to builder. Plan page wiring: popup "Get your certificate" primary btn + permanent cert link in the win card post-completion (`certHref` uses the celebration date so revisits keep the original date). GA: cert_view/cert_name_set/cert_copy_link/cert_linkedin/cert_print. Sitemap +certificate/. Parse gates 3/3 both pages; empty state verified in-browser (preview strips query params, so full render verified LIVE post-push). **Remaining follow-ups: dark theme (V15), Excel plans.** Prior:

**Prior:** August 20, 2026 — 🏆 **v3.14.0: Plan Builder v2 — 3/5/7-day runs, wins, all of it research-grounded.** Same session as v3.13.0 (below), Mike's follow-up: "run it through all our guides, canon and bibles… add 5 and 7 day option, add successes and badges and fun shit, really research this." Method: 4 parallel research agents (canon audit vs CURRICULUM_STANDARD/DRILL_FIRST_CONCEPT/KIT_VERB_TESTS/DASHBOARD_STANDARD/LISTEN_STANDARD/sql-quest-learning-science-review · plan-design primary lit · badges/streaks primary lit · Duolingo/DataCamp/Codecademy/LinkedIn/fCC/100DaysOfCode scan), findings synthesized into the build; **the evidence now ships on the page** as a "Why the planner works the way it does" section (Gollwitzer &amp; Sheeran 2006, Milkman 2011, Nunes &amp; Drèze 2006, Sharif &amp; Shu 2017, Silverman &amp; Barasch 2023, Deci/Koestner/Ryan 1999). Build (full list in CHANGELOG 3.14.0): Q3 plan length 3/5/7 (5/7 carry ONE emergency skip, never rendered as failure), Q4 time-of-day anchor baked into day headers, new level `rusty` + new role `bi`, 7 authored days per level sliced by length w/ day-3 milestone strip, warm-up retrieval openers, endowed progress bar (setup picks count, reason stated), wins system (competence-worded, post-hoc, shelf+toast; NO leaderboards, NO daily streaks, NO pushed shares), completion popup w/ reduced-motion-aware confetti + copy-your-win, permalinks ?level=&amp;role=&amp;days=. Canon fixes: next-bar 17px/700, per-combo reset (V13), fork lines, prequestions in why-lines, learner-generated win before reveal, `data-listen-root` ready. localStorage v1→v2 key migration (level:role → level:role:3). **Verified in-browser pre-push:** 5-day finance build, skip on day 5, full completion → popup + 3 badges, zero console errors; parse gate 3/3. **Flagged follow-ups: dark theme (verb test V15, planner is light-only), certificate page (v3 candidate), Excel plans.** Prior:

**Prior:** August 20, 2026 — 🗺️ **v3.13.0: NEW SECTION `plan/` — the Three-Day SQL Plan builder.** Mike's pitch: "you want to be better at SQL, for what role, here's a 3-day win… tells them why we are thinking how we are thinking." Scope calls (delegated to Claude): lives on APK (it routes existing traffic into the kits/drill/guides; a standalone app would start from zero audience) and v1 is SQL-only with per-role analysis twists (prove the format before Excel). **It is a router, not a course:** two chip questions (4 levels: brandnew/basics/interview/onthejob × 5 roles: data/marketing/finance/ops/unsure) → hand-authored 3-day plan, every day ≤1 hr. Steps deep-link real surfaces only — `sql/#lesson-<id>`, `drill/?ladder=<slug>` (music-country/grouping/joins/telco/steam), `guides/<slug>/`, `interview/`, SQL Dry Run; interview day 3 sends them to real job posts ("the posting is the syllabus"). Reasoning is the product: per-step "why" line + per-day "The thinking" box + plan-level "How this plan was thought through." Role twists = TWISTS map (funnel/cohort for marketing, MoM/running-total for finance, reconciliation/dupes for ops, CASE-segments for data; unsure = data + a what-is-data-analysis read on day 1). "Deal a random plan" never re-deals the same pair. Progress: `localStorage['apk-plan-v1']`, done-map keyed `level:role` so switching answers never wipes progress; pinned bottom next-up bar (complete state goes green). GA: `plan_built`/`plan_step`/`plan_complete`. Hub: new "Start with a plan" group atop the Other-ways card (no `data-kit`, stays out of the progress renderer). Sitemap +`plan/` (Edit tool, not PowerShell — BOM rule). Parse gate 3/3 OK; live URL verified post-push. **v2 candidates, pending Mike's read: Excel day-plans, an Excel+SQL combined level, plan permalinks (`?level=&role=`).** Prior:

**Prior:** July 15, 2026 — 🎯 **v1.131.0: JOIN Lab reworked to challenge-first (Mike follow-up on v1.130.0).** Focus = terminal + "try these on your own"; rest optional. Removed the "Examples to try now" grid; each of the 4 `JOIN_TASKS` is now its own terminal (`jc-editor-<i>`/`runJoinChallenge`). **Progressive scaffold, no hint button:** static "give it 3 tries" callout; every wrong Run escalates via `_labFillPartial` — miss 1 fills 1/3 of the answer + 1 grey help line, miss 2 fills 2/3 + 2 lines, miss 3 fills the full answer + 3 lines; the persistent grey `jc-help-<i>` block grows one line per miss; correct run greens + resets `_jcAttempts[i]`. Schema diagram + INNER-vs-LEFT walkthrough demoted to collapsed "Reference" cards (`toggleJoinSchema`/`toggleJoinCompare`); one-line table summary stays inline. Removed dead `JOIN_EXAMPLES`/`runJoinExplorer`/`loadJoinExample` etc. **Verified in a local browser** (served kit, drove challenge 0): miss1/2/3 → editor 1/2/3 answer-lines + 1/2/3 help lines, correct run resets. Parse 3/3. Prior:

**Prior:** July 15, 2026 — 🔗 **v1.130.0: SQL kit playtest-triage batch — JOIN Lab redesigned + new COUNT guide.** From Mike's playtest-tracker feedback. (1) **JOIN Lab → "JOIN Explorer"** (`sql/index.html`, `renderLabJoin`): Mike found the old one-task-at-a-time pager confusing ("the little dots that switch the screen... very hard to know you're supposed to swipe there... overcomplicated"). Rebuilt flat, everything visible: one live terminal (`#join-editor` + `runJoinExplorer()`), a **"Examples to try now"** grid of 6 ready-to-run joins (`JOIN_EXAMPLES`, one-click `loadJoinExample(i)` loads+runs+shows a "what you'll see" line), the INNER-vs-LEFT **walkthrough** (`renderJoinCompare`, now open by default), and **"Now try these on your own"** = the 4 `JOIN_TASKS` as open challenges with a per-challenge **Help** panel (`toggleJoinHelp`, worked answer + `loadJoinAnswer`). Removed dead `_joinTaskIdx`/`runJoinQuery`/`toggleJoinHint`; Aggregation Lab's shared `_runLabQuery` untouched. Parse gate 3/3 OK. (2) **New guide `guides/sql-count-function/`** ("COUNT in SQL") — Mike asked for a knowledge article on COUNT(*) vs COUNT(column) vs COUNT(DISTINCT), the recall question the kit asks, plus COUNT for data-migration verification. Cloned the guide template; linked from the GROUP BY lesson `notes`, `sitemap.xml`, and the 7 uniform guide footers. **⚠️ Not yet playtested by Mike — the JOIN Explorer especially wants his feel pass (this is a UX redesign of a live surface).** No in-app version string exists; version lives in CHANGELOG.md only. Prior:

**Prior:** July 12, 2026 — 📣 **v1.126.0: gems-style "Say It Out Loud" rewrite, all 7 lesson kits.** Mike's ask: the line-by-line read-out-loud he wrote in the Steam Hidden Gems portfolio SQL (`apk-portfolio-hidden-gems/queries/hidden_gems.sql` — "SELECT (display) the Name column", "WHERE (keep only) rows with at least 2,000 reviews") is better than the kits' RAL blocks, which were often loose vocabulary lists untied to the displayed query. 7 parallel content agents (one per kit file) rewrote `ral.lines` ONLY: chunks = real fragments of the displayed code in order, descs = plain-verb-gloss readings with concrete values, functions broken down on first appearance in lesson sequence then phased out, ≤2 essential vocab rows kept per lesson, conceptual rals untouched, `say` untouched. Coverage: SQL 45 · PBI 20 · Excel 12 · Tableau 10 · Chart Literacy 9 · Forecasting 5 · Python 5 (Python was mostly already in style). Suites: parse gate 7/7 · de-test 120/121 (the 1 fail — SQL L105 "chart card eyebrow" — pre-exists on the untouched tree, verified via git stash; NOT from this change) · legacy 24 · render-smoke clean. ⚠️ This file's "Current version: v1.118.0" line below was stale — v1.119–1.125 shipped in other sessions (see CHANGELOG.md); repo is now at **v1.126.0**. Prior:

**Prior:** July 7, 2026 (continued) — 📖 **v1.116.0–v1.118.0: the three items flagged at v1.115.0, Mike's "go for all".** (1) **v1.116.0 Final Exam fill→MC:** the exam's 3 typed questions (e3 COUNTIF / s4 ORDER BY+LIMIT / p4 groupby) converted to tap-the-choice w/ hand-authored distractors (correct at index 2/0/1 — final/ uses fixed `correct:` index, NO shuffle); fill machinery deleted (input branch, answerFill, regex grading, `.fill-input` CSS, orphaned escapeHtml/escapeAttr); boot migration drops non-numeric saved answers so old typed answers can't grade wrong forever. (2) **v1.117.0 Tableau What's-Wrong charts:** 8 diagnostic items (ids 2,3,7,9,17,18,19,20) got `svg:` keys + a `TAB_WRONG_SVG` inline-SVG map rendered in `renderWrong` (Excel's CHART_SVG pattern; zero-dep, CSS-var themed). **Scope call closing the all-exercise chart audit (Medium #6 remainder): chart-CHOICE questions ("which chart fits?") stay text-only ON PURPOSE — the visual would reveal the answer.** Inventory agent found no text-only diagnostic chart items in SQL/Python/PBI/Stats exercise banks. (3) **v1.118.0 GR-D COMPLETE:** 6 parallel content agents (one per kit file) swept every `exp:`/`explain:` + `notes:` — ~730 reviewed, 92 rewritten opening-sentence-only to lead w/ a real-world analogy or concrete scenario (Excel 25 · Stats 16 · Tableau 15 · PBI 15 · SQL 11 · Python 10); `say` lines exempt (literal readings). Bonus content fix: Tableau Top-N quiz exp said Top N evaluates AFTER other filters — corrected (Top N runs before dimension filters; that's why context filters exist). **All suites green after each step: 51-script parse gate · de-test 121 · legacy 24 · render-smoke 13/13.** Test batches 35–36 below. **NEXT = the pay options (freemium gate work) — the finish-line/non-paywall backlog is now EMPTY; only Mike's playtest batches (27–36) and the paywall remain.** Prior:

**Prior:** July 7, 2026 — 🧹 **v1.115.0: FINISH-LINE POLISH SWEEP (pre-paywall stabilization, Mike-directed: "complete all non-paywall lesson/interface items, then a complete test for UX, content, deadlinks").** Closed every open non-paywall interface item: home "Lessons" section (per-unit bars + Next-up 3) rolled to SQL/PBI (template cards after Skill Readiness) + Python (static `home-lesson-units`/`home-next-lessons` divs, Excel-style) + Stats Next-up card; Stats readiness UN-FROZEN (was hardcoded `lesson{i*2+1}` keys → missed Unit 0 + L11–12; now live per-unit via numeric `unit` + `unames` map) + Resume label position-fixed; Excel resume card position fix (`lessonPos(cur.id)+1`, was raw id → "LESSON 101") + v2 Close stage now has lesson-bound `startGuided(lid)` "Practice this →" (was only generic "Practice drills", inconsistent with all 5 other kits' Done cards); mobile `pars-check` class + full-width CTA + chip wrapping in the ≤600px blocks (5 kits, `:has()` selectors); Chart Literacy/Forecasting cross-links in Excel/Python/Interview/Simulator/Final; dead `.fill-input`/`.drill-input` CSS deleted (6 kits — Final KEEPS its, the exam legitimately uses typed `fill-input`); README refreshed; sitemap +projects/. **TEST INFRA:** private harness (`C:/Users/Mike/Projects/apk-headless/`) modernized — `headless-test.mjs` got supabase_auth_sync + apkPass stubs (external `<script src>` never evals headlessly — getCurrentUser threw), obsolete flag-feature checks REMOVED (feature deleted v1.72.0), new drill-choices integrity check (>=2 choices, choices[0] non-empty), practice-this check walks the v2 Close stage via per-kit stage setters (`setPyStage` etc.); NEW `render-smoke.mjs` (13-page runtime boot + render + undefined/NaN leak check); `<select>` stub gained `options`/`selectedIndex`. **ALL GREEN: 51-script parse gate · de-test 121 · legacy 24 · render-smoke 13/13 · link crawl (17 externals HTTP 200, all internal resolve) · zero mojibake/template leaks.** Test batch 34 below. **NEXT = the pay options (freemium gate work), per Mike — or triage batch 27–34 playtest results first.** ⚠️ Flagged, not built: Final Exam's typed `fill-input` answers are the last free-text grading surface (exact-match risk — the drill audit deliberately scoped to drills); GR-D analogy remainder + all-exercise chart audit remain parked as their own cycles. Prior:

**Prior:** July 6, 2026 (continued) — 📊 **v1.110.0: Python + PBI track-lesson charts (Medium #6 slices 2+3) — track-chart work COMPLETE.** Generators copied per kit (`pyChartHTML` w/ --dim/--success/--err · `pbiChartHTML` w/ SQL-clone vars), `PY_CHARTS`/`PBI_CHARTS` 7 lessons each, spec numbers match each kit's OWN tables (they differ between kits on purpose — 702 margins 60/23/15 in Python vs 60/22/14 elsewhere; 803 PBI = histogram bins not box plot). Render after `lessonOutputHTML`/`lessonPbiHTML` in v2 stage 1 (⚠️ each kit has a LEGACY viz render line too — the v2 one is the 6-space-indented one inside the stage-1 template). Harness charts config now on sql/python/powerbi; **121 checks green**. Test batch 33 below. **Medium #6's track-lesson slice is done; the broader all-exercise chart audit remains parked. NEXT per roadmap: Mike's playtest backlog (batches 27–33), GR-D analogy remainder, or GROWTH.md.** Prior (same day):

**Prior:** July 6, 2026 (continued) — 📊 **v1.109.0: Medium #6 slice 1 — SQL track lessons get real inline-SVG charts.** New parametric generators in `sql/index.html` (`svgLineChart`/`svgBarChart` w/ diverging support/`svgParetoChart`/`svgBoxPlot` + `sqlChartHTML` card), `SQL_CHARTS` map (7 lessons: 603/604/605/702/705/803/804 — spec numbers MATCH each lesson's viz table), rendered in Worked Example after `lessonResultHTML`. Reuse these generators for the Python/PBI slices (copy + kit-var rename; Python uses --dim not --muted). Harness: `de-test.mjs` gained a per-kit `charts:` config + an **apkPass stub** (external `assets/apk-pass.js` isn't eval'd headlessly — without the stub, premium id≥500 lessons render the GATE and hide render bugs; leak regex tightened to bare `>undefined<` after a legit-prose false positive). **117 checks green.** Test batch 32 below. **NEXT #6 slices: Python then PBI track-lesson charts.** Prior (same day):

**Prior:** July 6, 2026 (continued) — 🏁 **v1.107.0 + v1.108.0: Tableau + Stats Phase D+E parity — D+E ROLLOUT COMPLETE, all 6 lesson kits on the full learning-science stack.** Tableau (v1.107.0): `S`/`openLesson`+`lessonState`, renders into `view-lessons`, keys `tpk-*`, `TAB_REINFORCES` 32×2, tour button + `prereq` fallback preserved on Orient, greeting in the Home resume card (already position-based). Stats (v1.108.0): ⚠️ numeric `unit` (bridge condition `l.unit!==0`, NOT startsWith), section-based (no `ral`), quiz explanation field = `explain` not `exp`, keys `spk-*`, `STAT_REINFORCES` 16×2, greeting under the Home header. Harness: `de-test.mjs` now covers all 5 ported kits (homeEl config for view-home kits, numeric-unit tolerance, exp|explain) — **115 checks green**. Test batches 30–31 below. **Sprint total: 175 lessons across 5 kits given spaced recall in one day (165 cue sets authored). NEXT: Mike plays batches 27–31; then the roadmap's next item.** Prior (same day):

**Prior:** July 6, 2026 (continued) — ✅ **v1.106.0: Power BI kit Phase D+E parity** (1:1 SQL-clone port: `state`/`navigate`/`renderLesson(id)`, `_syncRecalls` hooked in renderLesson, keys `pbikt-*`, `PBI_REINFORCES` 39×2 cues, Lesson 1 tour button preserved on Orient, resume position fix, reset clears keys). `de-test.mjs` KITS config gained `powerbi`; **69 checks green (SQL+Python+PBI)**. Test batch 29 below. **NEXT = Tableau → Stats** (⚠️ Tableau: `S` state, `ral:[{shelf,lines}]`, Chart.js re-invoke after stage injection; Stats: section-based, no `ral` — extend the relevance haystack with `sections`). Prior (same day):

**Prior:** July 6, 2026 (continued) — ✅ **v1.105.0: Python kit Phase D+E parity** (same recipe as SQL below, adapted to Python's Excel-shaped architecture: `S`/`save`/`openLesson` single entry — `_syncRecalls` hooks openLesson, NOT renderLesson; static Home resume card got a `#returnGreeting` element + `checkReturnStreak()`; keys `ppk-recalls`/`ppk-recall-wins`/`ppk-last-visit`; `PY_REINFORCES` 42×2 cues; resume num/watermark now POSITION not raw id; both reset paths clear recall keys — the all-kits reset also clears epk-/sqlkit- recall keys). Headless suite generalized: `C:\Users\Mike\Projects\apk-headless\de-test.mjs` (kit-parameterized KITS config — add each future kit there; 46 checks green SQL+Python; its cue-relevance heuristic caught one weak cue set, rewritten). Test batch 28 below. **NEXT = Power BI → Tableau → Stats, same recipe** (⚠️ Stats is section-based, no `ral` — the cue-relevance haystack may need `sections` added; Tableau uses `S` + `ral:[{shelf,lines}]`). Prior (same day):

**Prior:** July 6, 2026 (later same day) — ✅ **v1.104.0: SQL kit Phase D+E learning-science parity with Excel** (sprint kicked off from the ROADMAP's Learning-Science item; Mike's call: Excel+SQL first — Excel turned out already complete at Phases A–F, so the real gap was SQL's missing D+E layers). Shipped into `sql/index.html`: **spaced-recall engine** (+1/+3/+7 positions; queue on "Not yet"/"Almost" rating, "Quick recall — no peeking" card at Orient, got-it/nope buttons, keys `sqlkit-recalls`/`sqlkit-recall-wins`; dequeue is lesson-change-guarded via `_syncRecalls` because SQL re-renders per stage — do the same in every future port), **`SQL_REINFORCES` map** (46 lessons × 2 hand-authored retrieval questions, single auditable block + boot merge onto `l.reinforces` — pattern of record for the Python/PBI/Tableau/Stats ports), **artifact bridge** at Orient (same-unit chains only), **retrieval-honesty note** on Try, **recall-wins counter** in Close, **return-visit greeting** on Home (`sqlkit-last-visit`). Ride-alongs: resume card shows lesson POSITION not raw id; coach key migrated to shared `apk-coach-key` (legacy `sql-coach-key` read as fallback — cleanup item closed); reset-all clears new keys. **Testing = backlogged per Mike (build-sprint mode); new private headless suite `C:\Users\Mike\Projects\apk-headless\sql-de-test.mjs` (23 behavioral checks, all green; covers eval, cue coverage/relevance, 46-lesson field integrity, recall round-trip, 46×7 stage renders, bridge/note/greeting, links). Test batch 27 below for Mike.** Backup tag pre-sprint: `backup-v1.103.0-pre-learning-science`. **NEXT = roll D+E to Python → Power BI → Tableau → Stats (reuse this exact recipe), pending Mike's go.** Prior (same day):

**Earlier same day:** July 6, 2026 — ✅ **NEW SECTION shipped (v1.103.0): Project Studio (`projects/`)** — portfolio-project builder, Mike's direct ask (vision: "helps you clarify your project, build it, and present it while teaching real analyst workflows... focus on what skills will land them a job first"). Four phases per project: **Scope** (7-step wizard ported from the artifact `portfolio-scoping-kit.jsx` in `C:\Users\Mike\Projects\apk-portfolio-hidden-gems\handoff_bundle\`, + a new tools step with Excel/SQL/Tableau recommended-core guidance) → **Plan** (step-by-step from 3 sources: guided template built from scope+tools · paste-your-own parser (`## Stage` + `[skill]` tags) · optional Claude BYOK draft on Sonnet via shared `apk-coach-key`; all land in one editor, then lock) → **Build** (accordion checklist over Ask→Prepare→Process→Analyze→Share→Act with per-stage best-practice mentor notes + per-step notes) → **Present** (scope-seeded deliverable checklist + .md project-doc export). State = `localStorage['apk-projects-v1']`; hub card added (deliberately no progress/continue tracking). **Dogfood plan: Mike runs his Steam Hidden Gems analysis through it** (dataset: `apk-portfolio-hidden-gems/handoff_bundle/games.csv`, 126k rows — owners ranges need midpoint parsing; thresholds from real percentiles). His feedback drives the next iteration. Not yet built from the original hidden-gems handoff: SQLite load pipeline, scoring SQL, Tableau dashboard, Excel pivot leg, docx template, scope→starter-SQL generation.

**Prior session:** July 5, 2026 — ✅ **Medium #5 DONE (v1.102.0): premium-unlocked badges.** Buying now *feels* like unlocking. **Source of truth = genuine redemption (Mike's call), NOT the pre-Aug-1 free bypass** — since `apkPass.isUnlocked()` returns true for everyone until launch, it can't drive an honest "you unlocked this" badge. All logic centralized in `assets/apk-pass.js`: new **`hasRedeemed()`** (real local unlock: redeemed unexpired code), **`isMember()`** = `hasRedeemed()` OR account entitlement (`hasInterviewPass()`, async-hydrated + cached in `_acctPass`), **`lessonBadge()`** (3-state pill: ✓ Unlocked / Premium · free now / 🔒 Premium), **`unlockedBanner(noun)`**, **`celebrateUnlock()`** (one-time toast; `apk-pass-celebrated` flag shared across the suite so it fires once total), **`hydrateAcct(rerender)`**. Applied to the 4 tool kits w/ interview tracks (Excel/SQL/Python/Power BI — card pill + lesson banner + init) + fully-gated Final Exam (home banner + init). **Tableau/Stats have NO premium content (no id≥500 tracks, don't load apk-pass.js) — correctly untouched.** Interview kit (free) + Simulator (BYOK) aren't pass-gated. Parse gate clean (5 kits + shared JS). **➡️ NEXT Medium item = #6 real chart/pivot visuals in exercises** (LARGE/phased, Excel first — save for last or split). Then #7 per-kit SEO (post-GSC, deferred).

**Prior (same day):** ✅ **Medium #4 DONE (v1.101.3): retired stale "no login / no telemetry" copy.** Auth+sync + GA4 are live, so those claims are false. Reframed README, chart-literacy + forecasting meta descriptions ("No install, free to start") + footers ("saved in this browser, and syncs across devices when you sign in"), dropped the false "No telemetry" chip from final + simulator footers (both load GA4), and dropped "No sign-up" from the hub arcade note. The hub hero already said "Sign in to sync across devices" (unchanged). Parse clean across all touched files. **✅ Follow-up shipped same session (v1.101.4): privacy.html + terms.html rewritten to be accurate about optional sign-in + Supabase sync** — no-account use stays local; account stores email + synced progress/entitlements (Supabase named as backend); added account-deletion to "Your choices" + auth/sync to providers; both "Last updated" → July 5, 2026. Facts sourced from `SUPABASE_INTEGRATION.md`. **➡️ NEXT Medium item = #5 premium-unlocked badges** (badge/"Premium" pill driven by entitlement state — `user_entitlements`/`hasInterviewPass()`; pairs with freemium launch; verify current state first — #2/#3/#4 were all partly pre-built). Then #6 real chart visuals (large, last), #7 SEO (post-GSC).
...
**➡️ NEXT = work the MEDIUM bucket** (Mike's call, July 5 — the High quick-fixes #1–#6 are all done; the remaining High items are big gated efforts, deferred). Same cadence as the High run: **do ONE Medium item at a time, build + verify (headless parse gate: `node` new-Function on each kit's inline `<script>`) + ship (commit + push, per-kit checkpoints), then hand off.** Mike tests independently via the private `testing/APK_Testing_Checklist.xlsx`; don't walk him through in chat unless asked. Skip local preview (GH-Pages — he checks live). Commit as Michael Nocito, no AI trailers.
  **Medium bucket, in suggested order (small/high-value first — full detail in `ROADMAP.md` 🟡 section):**
  ...
  5. ✅ **DONE (v1.102.0)** — Premium-unlocked badges. Genuine-redemption source of truth (`hasRedeemed()`/`isMember()` in `apk-pass.js`, not the free bypass); 3-state card pill + unlocked banner + one-time celebration toast; applied to Excel/SQL/Python/Power BI + Final Exam. Tableau/Stats have no premium content (untouched).
...
- AI Coach key SHARED with Excel + Python + Power BI (`apk-coach-key`); SQL still separate (`sql-coach-key`) — ⚠️ migrated to the shared key in v1.104.0
...
- AI Coach key SHARED with Excel + Python + Power BI + Tableau (`apk-coach-key`); SQL still separate (`sql-coach-key`) — ⚠️ migrated to the shared key in v1.104.0

### Batch 27 — SQL kit: Phase D+E layer (v1.104.0, untested)
All on https://michaelnocito.github.io/analyst-prep-kit/sql/ — start from a reset kit (Settings → Reset all) so the queue starts empty. ⚠️ SQL lesson POSITION = list order: Unit 0 = Lessons 1–4 (ids 101–104), so "Lesson 5" = SELECT & FROM (id:1).
- **104a — recall queues on a shaky rating:** Open Lesson 5 "SELECT & FROM", walk to the Done stage, rate it **"Not yet"**. Open Lesson 6 "WHERE" — the Orient stage must show a **"Quick recall — no peeking"** card listing two SELECT/FROM questions.
- **104b — got-it counts a win:** On that card tap **"I remembered it"** — card disappears. Finish any lesson to its Done stage: it must show **"1 concept recalled from memory so far"**.
- **104c — nope re-queues:** Rate another lesson "Not yet", advance one lesson to see its recall card, tap **"Nope, remind me again"** — the card must reappear on the NEXT lesson you open (one position later).
- **104d — Have-it clears:** Re-open a "Not yet" lesson, go to Done, re-rate it **"Have it"** — its recall cards must stop appearing in later lessons.
- **104e — bridge + honesty note:** Open Lesson 6 "WHERE" (same unit as Lesson 5): Orient must show **"Last time you built: SELECT & FROM. Now you'll add to it."** Unit openers (e.g. Lesson 5, first of Unit 1) must NOT show it. The Try stage must show the italic **"This feels harder because it works"** line.
- **104f — greeting + resume number:** Tomorrow, reopen the kit Home: the Continue Learning card shows **"Good to have you back. Pick up where you left off."** (not on a same-day revisit) and says **"Lesson N"** by position (Lesson 1–4 for Unit 0), never "Lesson 101".
- Cross-cut: AI Coach key is NOW SHARED (`apk-coach-key`) — a key saved in Excel's Settings must work in SQL's coach buttons (legacy `sql-coach-key` still honored if it was already set).

### Batch 28 — PYTHON kit: Phase D+E layer (v1.105.0, untested)
Same walk as batch 27, on https://michaelnocito.github.io/analyst-prep-kit/python/ — reset the kit first. ⚠️ Python POSITION: Unit 0 = Lessons 1–4 (ids 101–104), so "Lesson 5" = Variables & Data Types (id:1).
- **105a — recall queues on a shaky rating:** Open Lesson 5 "Variables & Data Types", reach Done, rate **"Not yet"**. Open Lesson 6 "Lists & Dictionaries" — Orient shows the **"Quick recall — no peeking"** card with two variables/types questions.
- **105b — got-it counts a win:** Tap **"I remembered it"**; any lesson's Done stage then shows **"1 concept recalled from memory so far"**.
- **105c — nope re-queues:** Tap **"Nope, remind me again"** on a recall card — it reappears on the next lesson you open.
- **105d — Have-it clears:** Re-rate the source lesson **"Have it"** — its cards stop appearing.
- **105e — bridge + honesty note:** Lesson 6 Orient shows **"Last time you built: Variables & Data Types."**; unit openers don't. Try stage shows the italic **"This feels harder because it works"** line.
- **105f — greeting + resume number:** Next day, Home's resume card shows **"Good to have you back."** under the lesson title, and the LESSON number chip + watermark show position (1–4 for Unit 0), never "101".

### Batch 29 — POWER BI kit: Phase D+E layer (v1.106.0, untested)
Same walk as batches 27–28, on https://michaelnocito.github.io/analyst-prep-kit/powerbi/ — reset the kit first. ⚠️ PBI POSITION: Unit 0 = Lessons 1–4 (ids 101–104), so "Lesson 5" = The Power BI Interface (id:1).
- **106a — recall queues on a shaky rating:** Open Lesson 6 "Connecting to Data" (id:2), reach Done, rate **"Not yet"**. Open Lesson 7 "Building Your First Visual" — Orient shows the **"Quick recall — no peeking"** card with two connecting-to-data questions.
- **106b — got-it + wins:** Tap **"I remembered it"**; any Done stage then shows **"1 concept recalled from memory so far"**.
- **106c — nope re-queues / Have-it clears:** "Nope" makes the card reappear on the next lesson; re-rating the source lesson "Have it" stops its cards.
- **106d — bridge + honesty + tour button intact:** Lesson 6 Orient shows **"Last time you built: The Power BI Interface."**; Lesson 5 (id:1, "The Power BI Interface") Orient must STILL show the **"Tour the workspace interactively →"** button; Try stage shows the italic honesty line.
- **106e — greeting + resume number:** Next day, Home's Continue Learning card shows **"Good to have you back."** and **"Lesson N"** by position, never "Lesson 101".

### Batch 30 — TABLEAU kit: Phase D+E layer (v1.107.0, untested)
On https://michaelnocito.github.io/analyst-prep-kit/tableau/ — reset the kit first. ⚠️ Tableau POSITION ≠ id: Lesson 5 = "The Tableau Interface" (id:1), Lesson 7 = "Dimensions vs. Measures" (id:2), Lesson 8 = "Your First Bar Chart" (id:3).
- **107a — recall queues:** Open Lesson 7 "Dimensions vs. Measures", reach Done, rate **"Not yet"**. Open Lesson 8 "Your First Bar Chart" — Orient shows **"Quick recall — no peeking"** with two dimension/measure questions.
- **107b — got-it / nope / Have-it:** "I remembered it" → Done stage shows **"1 concept recalled from memory so far"**; "Nope" re-queues to the next lesson; re-rating "Have it" stops the cards.
- **107c — bridge + honesty + tour intact:** Lesson 7 Orient shows **"Last time you built: Your First Drag."**; Lesson 5 (Interface) Orient still shows the **workspace tour button**; Try stage shows the italic honesty line. Worked Example charts must still DRAW (Chart.js canvas not blank).
- **107d — greeting:** Next day, the Home resume card shows **"Good to have you back."**

### Batch 31 — STATS kit: Phase D+E layer (v1.108.0, untested)
On https://michaelnocito.github.io/analyst-prep-kit/stats/ — reset the kit first. ⚠️ Stats POSITION: Unit 0 = Lessons 1–4 (ids 101–104), so Lesson 5 = "Mean, Median & Mode" (id:1).
- **108a — recall queues:** Open Lesson 5 "Mean, Median & Mode", reach Done, rate **"Not yet"**. Open Lesson 6 "Variance & Standard Deviation" — Orient shows **"Quick recall — no peeking"** with two center-measure questions.
- **108b — got-it / nope / Have-it:** same three behaviors as the other kits (wins counter reads "N concepts recalled from memory so far" on Done).
- **108c — bridge scoped to real units:** Lesson 6 Orient shows **"Last time you built: Mean, Median & Mode."**; Unit-0 lessons (1–4) must NOT show a bridge; Try stage shows the italic honesty line. Distribution Lab + Worked-Example charts must still draw.
- **108d — greeting:** Next day, Home shows **"Good to have you back. Pick up where you left off."** under the header.

### Batch 32 — SQL kit: track-lesson charts (v1.109.0, untested)
On https://michaelnocito.github.io/analyst-prep-kit/sql/ — all in the Worked Example (2nd) stage, in a "See the chart" card BELOW the "See it on screen" result table. Check dark AND light mode on at least one.
- **109a — line + funnel:** Open "Month-over-Month Growth with LAG" (Unit 5) → Worked Example: a **line chart of monthly revenue (Jan 180 → Jun 268)** with the March dip visible. Then "Funnel Conversion" → **4 shrinking bars (12,000 → 760)**.
- **109b — verdict colors:** Open "Budget vs Actual: Variance Analysis" (Unit 6) → diverging bars where **Revenue +80 is GREEN but COGS +45 is RED** (color = verdict, not sign; caption explains).
- **109c — box plot + Pareto:** "Distribution & Outliers" (Unit 7) → box plot with the **$540 outlier dot in red**; "Pareto: Find the Vital Few" → bars A–F with an **amber cumulative % line ending at 100%**.

### Batch 33 — PYTHON + POWER BI: track-lesson charts (v1.110.0, untested)
Same "See the chart" card below the output table in the Worked Example stage. Spot-check one per kit in dark AND light:
- **110a — Python:** https://michaelnocito.github.io/analyst-prep-kit/python/ → "Month-over-Month Growth with pct_change" (Unit 5) shows the **revenue line (Jan 180 → Jun 268)**; "Budget vs Actual" (Unit 6) shows **Revenue +80 GREEN / COGS +45 RED** diverging bars; "Distribution & Outliers" (Unit 7) shows the box plot with a **$510 red outlier dot**.
- **110b — Power BI:** https://michaelnocito.github.io/analyst-prep-kit/powerbi/ → "Month-over-Month Growth with DATEADD" shows the line chart; "Distribution & Outliers" shows a **histogram (bins $0–$200+, peak at $40)**; "Pareto / ABC Analysis" shows bars + **amber cumulative line ending at 100%**.

### Batch 34 — Finish-line polish sweep (v1.115.0, untested)
- **115a — SQL home Lessons card:** https://michaelnocito.github.io/analyst-prep-kit/sql/ → Home. Below Skill Readiness, a **"Lessons"** card lists every unit with a progress bar + count, then **"Next up"** with 3 lesson rows; tapping a row opens that lesson.
- **115b — Power BI home Lessons card:** same check on /powerbi/.
- **115c — Python home Lessons card:** same check on /python/ (card sits between Skill Readiness and "Read It Aloud").
- **115d — Stats home:** Skill Readiness now shows the 4 real units (incl. "Unit 0: Before You Calculate") and moves when you finish a Unit-0 lesson; a **"Next up"** card lists 3 lessons; the Resume button says the lesson POSITION (never "Lesson 101").
- **115e — Excel resume + Close:** with your progress on a Unit-0 lesson, Home's Continue card shows the position number (1–4), not 101–104. Finish any lesson with drills to the Done stage: the actions row now leads with **"Practice this →"** (opens THAT lesson's guided drills), then "Next lesson →".
- **115f — Mobile Parsons (phone, any kit's Practice tab → Parsons):** the Check/Check Order button is full-width and thumb-height; long chips wrap instead of clipping off-screen.
- **115g — Cross-links:** Excel/Python/Interview/Simulator/Final home/footer "Other Kits" lists now include **Chart Literacy** and **Forecasting**; links work.
- **115h — Final Exam still types:** /final/ → any fill-in question still shows a typed input styled correctly (its CSS was kept while dead copies were removed elsewhere).

### Batch 35 — Final Exam fill→MC + Tableau What's-Wrong charts (v1.116.0–v1.117.0, untested)
- **116a — Exam MC:** https://michaelnocito.github.io/analyst-prep-kit/final/ → take the Excel section: question 3 (COUNTIF) is now multiple-choice (4 options, no typed input anywhere in the exam). Same for SQL q4 (ORDER BY/LIMIT) and Python q4 (groupby). Wrong pick reveals the correct option + explanation on section submit.
- **116b — old progress safe:** if you had a previously-typed answer saved, the exam loads clean (no crash, question just shows unanswered).
- **117a — Tableau broken-chart drills:** https://michaelnocito.github.io/analyst-prep-kit/tableau/ → Practice → What's Wrong: the flat-bars item (all bars same height) now shows a small bar chart ABOVE the options illustrating the symptom. Spot-check 3 more: truncated Y-axis (line chart, axis starts at 800), one-dot scatter, equal-tile treemap. Check dark AND light — charts use theme vars.
- **117b — chart-choice questions unchanged:** quiz items asking "which chart type fits?" still show NO chart (deliberate — the visual would give the answer away).

### Batch 36 — GR-D analogy openers, all 6 kits (v1.118.0, untested)
Spot-read voice + accuracy; these are content-only edits (92 strings).
- **118a — SQL:** open Lesson 6 "WHERE" → Quick Check → answer → explanation opens with the bouncer-at-the-door line. LEFT JOIN lesson's quiz opens with the class-roster/sign-up-sheet line.
- **118b — Stats:** Hypothesis Testing quiz opens with the courtroom line; a p-value explanation opens with "how weird is 9 heads in 10 flips?".
- **118c — Excel:** VLOOKUP lesson Gotcha opens with the receipt line; error-codes Gotcha opens with dashboard-lights.
- **118d — PBI:** CALCULATE Gotcha opens with "the room a question is asked in"; TOTALYTD Gotcha opens with the trip odometer.
- **118e — Python:** deep-copy quiz opens with "two name tags on one box"; pd.cut Gotcha opens with fence posts/panels.
- **118f — Tableau Top-N fix:** Filters lesson — the Top-N quiz explanation now says Top N calculates BEFORE dimension filters unless you add context (no longer contradicts the build step).
- **118g — voice check (subjective):** read any 5 rewritten openers — calm mentor voice, no exclamation marks, no cutesiness. Flag any that feel off; they're one-line fixes.
