...
"SELECT":{tries:[
 ...
 {p:"Return every column from sales (peek at the table).",sql:"SELECT * FROM sales;"}]},
"FROM":{tries:[
...
"INNER JOIN":{tries:[
 {p:"Each sale's amount with its product name.",sql:"SELECT p.name, s.amount FROM sales s INNER JOIN products p ON s.product_id = p.product_id;"},
 {p:"Each sale with the customer's name (customers table).",sql:"SELECT c.name, s.amount FROM sales s INNER JOIN customers c ON s.customer_id = c.id;"}]},
"LEFT JOIN":{tries:[
 {p:"ALL customers_old rows with any sales they made — keep customers with none.",sql:"SELECT co.name, s.sale_id FROM customers_old co LEFT JOIN sales s ON co.id = s.customer_id;"},
 {p:"All products with their sales; a product with zero sales would still appear.",sql:"SELECT p.name, s.sale_id FROM products p LEFT JOIN sales s ON p.product_id = s.product_id;"}]},
"ON":{tries:[
 {p:"Join sales to products — the ON clause connects product_id to product_id.",sql:"SELECT p.category, s.amount FROM sales s JOIN products p ON s.product_id = p.product_id;"},
 {p:"Join sales to customers — this time ON connects customer_id to customers.id (different column names!).",sql:"SELECT c.name, s.sale_date FROM sales s JOIN customers c ON s.customer_id = c.id;"}]},
"GROUP BY":{tries:[
...
"CASE WHEN":{tries:[
 ...
 {p:"Bucket sales: 'big' if amount >= 100, 'medium' if >= 40, else 'small'.",sql:"SELECT amount, CASE WHEN amount >= 100 THEN 'big' WHEN amount >= 40 THEN 'medium' ELSE 'small' END AS size FROM sales;"}]},
"COALESCE":{tries:[
 {p:"Show region, but replace NULL with 'Unassigned'.",sql:"SELECT sale_id, COALESCE(region, 'Unassigned') AS region FROM sales;"},
...
"Subquery":{tries:[
 {p:"Sales above the average sale amount.",sql:"SELECT * FROM sales WHERE amount > (SELECT AVG(amount) FROM sales);"},
...
"ROW_NUMBER()":{tries:[
 {p:"Number every sale from biggest amount to smallest.",sql:"SELECT sale_id, amount, ROW_NUMBER() OVER (ORDER BY amount DESC) AS rn FROM sales;"},
...
"OVER (...)":{tries:[
 {p:"Each sale next to the overall average amount (no GROUP BY needed).",sql:"SELECT sale_id, amount, AVG(amount) OVER () AS overall_avg FROM sales;"},
 {p:"Running total of revenue by date.",sql:"SELECT sale_date, amount, SUM(amount) OVER (ORDER BY sale_date, sale_id) AS running_total FROM sales;"}]},
"PARTITION BY":{tries:[
 {p:"Number sales 1,2,3… restarting per region.",sql:"SELECT region, amount, ROW_NUMBER() OVER (PARTITION BY region ORDER BY amount DESC) AS rn FROM sales;"},
...
"Alias (AS)":{tries:[
 ...
 {p:"Table aliases: shorten sales to s and products to p in a join.",sql:"SELECT p.name, s.qty FROM sales AS s JOIN products AS p ON s.product_id = p.product_id;"}]},
"PRIMARY KEY":{tries:[
...
"FOREIGN KEY":{tries:[
 ...
 {p:"Follow the FK the happy direction: each sale with its customer's status.",sql:"SELECT s.sale_id, c.name, c.status FROM sales s JOIN customers c ON s.customer_id = c.id;"}]},
...
"RIGHT JOIN":{tries:[
 {p:"RIGHT JOIN keeps every row of the RIGHT table: all customers_old, matched or not.",sql:"SELECT s.sale_id, co.name FROM sales s RIGHT JOIN customers_old co ON s.customer_id = co.id;"},
 {p:"Same result flipped as a LEFT JOIN — the usual rewrite.",sql:"SELECT s.sale_id, co.name FROM customers_old co LEFT JOIN sales s ON s.customer_id = co.id;"}]},
...
"DATE_FORMAT(date, format)":{note:"DATE_FORMAT is MySQL. SQLite uses <code>strftime(format, date)</code> — %Y-%m gives '2024-03'.",tries:[
 {p:"Each sale's month as '2024-03' style.",sql:"SELECT sale_date, strftime('%Y-%m', sale_date) AS month FROM sales;"},
